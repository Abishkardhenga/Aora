...
export const GetPostDetailById = async (videoId) => {
    console.log("editing post id received backend ", videoId)
    try {
        const post = await database.getDocument(databaseId, videoCollectionId, videoId)
        return post;
...
export const deleteBookmark = async (bookmarkId) => {
    try {
        await database.deleteDocument(databaseId, bookmarkCollectionId, bookmarkId);
    } catch (error) {
        throw new Error(error);
    }
}