...
export const config = {
    ...
    bookmarkId: "66ae031c0034c9eefb1e",
    storageId: "66a89c5f0003d5e08566",
}



const { endpoint, platform, projectId, bookmarkId, storageId, databaseId, userCollectionId, videoCollectionId } = config;
...
export const createBookmark = async (userId, videoId) => {
    try {
        ...
        const response = await database.createDocument(databaseId, bookmarkId, ID.unique(), {
            userId: userId,
            videoId: videoId
        })
...
export const getBookmark = async (userId) => {
    ...
    try {
        const bookmarksResponse = await database.listDocuments(databaseId, bookmarkId, [
